...
public interface GrammarProgressMapper {
    ...
    // Retrieve all GrammarProgress entries for a specific user and course that are
    // not deleted
    ...
    // Retrieve a single GrammarProgress entry for a user and grammar that is not
    // deleted
    ...
    @Update("UPDATE grammar_progress SET is_finish_quiz = true, is_failed_quiz = false, last_modified = NOW() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
    void markGrammarQuizAsFinished(@Param("user_id") Integer userId, @Param("grammar_id") Integer grammarId,
            @Param("course_id") Integer courseId);

    @Update("UPDATE grammar_progress SET is_failed_quiz = true, last_modified = NOW() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
